...
$datos=isset($_GET['term'])?$_GET['term']:$_GET['tag'];

dropViews(array('view_friends'));
$value=$_GET['value']!=''?"u.id":"u.email";
//los que el usuario sigue detail as 'key',id as 'value'
CON::query("CREATE VIEW view_friends AS
	SELECT DISTINCT CONCAT(u.name,' ',u.last_name) AS `key`, $value AS value, l.id_friend AS id_friend
	FROM users u JOIN users_links l ON u.id=l.id_friend
	WHERE l.id_user=?",array($user));
//amigos
$dato=explode(' ',trim($datos));
$likes=array();
foreach($dato as $key){
	if($key) $likes[]="f.key LIKE '%%$key%'";
}
$likes=implode(' AND ',$likes);
$sql="SELECT
	DISTINCT f.key, md5(f.value) AS value
	FROM view_friends f JOIN users_links u ON f.id_friend=u.id_user
	WHERE u.id_friend=? AND ($likes) LIMIT 0,15;";

if(isset($_GET['term'])){
	$result=array();
	$query=CON::query($sql,array($user));
	while($array = CON::fetchAssoc($query)){
		$result[]= array('term'=>$_GET['term'],'id'=>$array['value'],'text'=>$array['key']);
	}
}else{
	$result=CON::getAssoc($sql,array($user));
}
