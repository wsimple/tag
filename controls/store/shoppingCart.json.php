			...
			if($product['seller']==$_SESSION['ws-tags']['ws-user'][id]){ $jsonResponse['datosCar2']['add'] = 'no'; }
				...
				if ($product['id_status']!='2' && $product['stock']>'0'){
					...
					if (!existe('store_orders_detail', 'id', "WHERE id_status = '11' AND id_order='".$idOrder."' AND id_product='".$producto."'")){
						...
						$_SESSION['car'][$producto]['category'] = constant($product['category']);
						$_SESSION['car'][$producto]['subCategory'] = constant($product['subCategory']);
...
//											$GLOBALS['cn']->query("UPDATE users SET description = 'aqui estoy 6' WHERE id ='".($id_comprador)."'");
						...
						notifications($car['order']['comprador'],$car['order']['order'],16,'','427');
						unset($_SESSION['car']);
					}else{
						 $datosCar = 'noCredit';
						 $price = 0;
		...
		case 9: //ordenes ya realizadas
			...
			while($array=  mysql_fetch_array($products)){
				...
				$datosCar[$a]['product_category']=  formatoCadena(constant($array['category']));
				$datosCar[$a]['product_subCategory']=  formatoCadena(constant($array['subCategory']));
