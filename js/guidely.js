var guidely = function()
{
	return {
		...
		welcome : function ()
		{
			...
			that.close();
			content = $('<div>', {
				'id' : 'guide-welcome', 'class' : 'guidely-guide'
			}).appendTo('body');
			popup = $('<div>', {
				'class' : 'guidely-popup'
			}).appendTo(content);
			...
			content.css(
			{
				'position' : 'absolute', 'top' : '75px', 'left' : '50%', 'margin-left' : '-' + content.outerWidth() / 2 + 'px'
			});
			close = $('<a>',
			{
		...
		clear : function ()
		{
			$('.guidely-guide, .guidely-number').remove();
			this._guides = [];
		},
		close : function ()
		{
		...
		_setGuidePosition : function (number, content)
		{
			...
			{
				left = (offset.left + number.outerWidth()) - 100;
				content.removeClass('guidely-anchor-right')
			}
			content.css({
				'position' : 'absolute', 'top' : offset.top + 25, 'left' : left
			})
		},
		_createStartTrigger : function ()
		{
			...
			$('<a>',
			{
				'href' : 'javascript:;', 'class' : 'guidely-start-trigger', 'html' : 'Start Tour', 'click' : function (e)
				{
					e.preventDefault();
					if (that._options.welcome) {
		...
		_getCoords : function (options)
		{
			...
			return {
				width : el.outerWidth(), height : el.outerHeight(), top : el.offset().top, left : el.offset().left
			};
		},
		_getAnchorCoords : function (options)
		{
			...
			offsetMap =
			{
				top_left : [coords.top - numberOffset, coords.left - numberOffset], top_right : [coords.top - numberOffset,
				(coords.left + coords.width) - numberOffset], top_middle : [coords.top - numberOffset,
				coords.left + (coords.width / 2) - numberOffset], middle_left : [coords.top + (coords.height / 2) - numberOffset,
				coords.left - numberOffset], middle_middle : [coords.top + (coords.height / 2) - numberOffset,
				coords.left + (coords.width / 2) - numberOffset], middle_right : [coords.top + (coords.height / 2) - numberOffset,
				coords.left + coords.width - numberOffset], bottom_right : [coords.top + coords.height - numberOffset,
				(coords.left + coords.width) - numberOffset], bottom_middle : [coords.top + coords.height - numberOffset,
				coords.left + (coords.width / 2) - numberOffset], bottom_left : [coords.top + coords.height - numberOffset,
