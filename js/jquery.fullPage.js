...
(function($){
	...
	$.fn.fullpage=function(options){
		var $cont=$(this===$.fn?'body':this);
		var opc=$cont.data('fullPage');
		var $main=$('html,body'),body=$('body');
		if(options==='remove'||options==='disable'||options==='close'){//if you want to disable the fullPage
			$(window).off('.fullPage');
			$(document).off('.fullPage');
			$('html,body').css({overflow:'',height:''});
			if(body.data('fullPageCSS')){
				body.css(body.data('fullPageCSS'));
				body.data('fullPageCSS',null);
			}
			return;
		}else if(options==='enableKeys'){
			enableKeyScroll();
		}else if(options==='disableKeys'){
			disableKeyScroll();
		}
		...
		$.fn.fullpage.setAutoScrolling=function(value){
			...
			if(!body.data('fullPageCSS'))
				body.data('fullPageCSS',{overflow:$main.css('overflow')});
			if(options.autoScrolling){
				...
				$main.scrollTop(element.position().top);
			}
		};
		...
		if($cont.find('#superContainer').length<1)
			$cont.wrapInner('<div id="superContainer"/>');
		//creating the navigation dots 
		if (options.navigation) {
			if($('#fullPage-nav').length<1)
		...
		$('.section').each(function(index){
			...
			if (numSlides > 0) {
				...
				$this.find('.controlArrow.next').css('border-color', 'transparent transparent transparent '+options.controlArrowUrl);
				$this.find('.controlArrow.prev').css('border-color', 'transparent '+options.controlArrowUrl + ' transparent transparent');
			...
			if(options.scrollOverflow){
				...
				$(window).on('load.fullPage', function() {
		...
		//when scrolling...
		if(!isTablet)
		$(window).on('scroll.fullPage',function(e){
			if(!options.autoScrolling){
		...
		*/
		/*
		$(document).on('touchmove.fullPage', function(event){
		...
		$(document).on('touchstart.fullPage',function(event){
			if(options.autoScrolling&&isTablet){
				var e=event.originalEvent;
		...
		*/
		...
		function addScrollEvent(){
			$cont
			.on('mousewheel.fullPage',MouseWheelHandler)
			.on('DOMMouseScroll.fullPage',MouseWheelHandler)
			.on('onmousewheel.fullPage',MouseWheelHandler);
		}
		$.fn.fullpage.moveSlideUp=function(){
			var prev=$('.section.active').prev('.section'),upScroll='';
			//upScroll = prev[0].id;
		...
		$(window).on('hashchange.fullPage',function(){
			...
			if(!isScrolling){
				...
				if(section&&section!==lastScrolledDestiny||(typeof slide!='undefined'&&(!isScrolling||force))){
					scrollPageAndSlide(section,slide);
				}
			}
		});
		...
		//navigation tooltips 
		$(document).on({
			'mouseenter.fullPage':function(){
				var tooltip = $(this).data('tooltip');
				$('<div class="fullPage-tooltip '+options.navigationPosition+'">'+tooltip+'</div>').hide().appendTo($(this)).fadeIn(200);
			},
			'mouseleave.fullPage':function(){
		...
		*/
		...
		$(window).on('orientationchange.fullPage',function(){
			doneResizing();
		});
		/**
