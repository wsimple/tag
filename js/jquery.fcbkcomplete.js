      ...
      function addMembers(etext, data) {
        ...
        if (data != null && data.length) {
          $.each(data, function(i, val) {
            ...
            cache.set(xssPrevent(val.value), xssPrevent(val.key));
      ...
      function bindEvents() {
        ...
        maininput.keydown( function(event) {
          ...
          if (event.keyCode != _key.backspace) {
			...
			//alert(event.keyCode);
          if ((event.keyCode == _key.enter || event.keyCode == _key.tab|| event.keyCode == _key.coma) && checkFocusOn()) {
            var option = focuson;
            addItem(option.text(), option.attr("rel"), 0, 0, 1);
