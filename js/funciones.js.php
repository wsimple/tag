...
function pageAction(action,data){
	...
	switch(action){
		...
		case 'profile'	:userProfile(opc[1]||'<?=js_string(USERPROFILE_PERSONALINFO)?>',opc[0]);break;
		case 'comment'	:
			commentTag(opc[1],opc[0]);
			//redir('?tag='+opc[0]);
		break;
		case 'video'	:openVideo(opc[0],opc[1]||null);break;
		case 'card'		:message('messages','<?=js_string(USERPROFILE_TITLEBUSINESSCARD)?>','','',450,300,DOMINIO+'views/users/account/business_card/businessCard_dialog.view.php?bc='+data,'');break;
		case 'share'	:shareTag('<?=js_string(MNUTAG_TITLESHARE)?>','tag='+opc[0]);break;
		case 'group'	:shareTag('<?=js_string(MNUTAG_TITLESHARE)?>','tag='+opc[0]);break;
		case 'sponsor'	:sponsorTag('views/tags/sponsor.php?type=4&tag='+opc[0],'<?=js_string(MNUTAG_TITLESPONSOR)?>',opc[0]);break;
		case 'report'	:bryTag('views/tags/report.php',8,opc[0],'<?=js_string(MNUTAG_TITLEABUSE)?>','timeLine');break;
		case 'editTag'	:redir('update?tag='+data);break;//actionsTags(10,opc[0]+'|0','controls/tags/actionsTags.controls.php','<?=$_SESSION['ws-tags']['ws-user']['code'].'|'.js_string(MNUTAG_ERRORNOISMYTAG).'|'.js_string(INDEX_ERRORHUBOHACK)?>');break;
		case 'createTag':
			if (opc[1]){
				$('#default-dialog').dialog('close');
				data=opc[0];
			}
			data=data?data:'';redir('creation'+data);break;//actionsTags(10,opc[0]+'|0','controls/tags/actionsTags.controls.php','<?=$_SESSION['ws-tags']['ws-user']['code'].'|'.js_string(MNUTAG_ERRORNOISMYTAG).'|'.js_string(INDEX_ERRORHUBOHACK)?>');break;
		case 'redeemPoinst':message('messages','<?=MNUUSER_REDEEMPOINTS?>','<div class="font_size5"><div><?php echo MAINMENU_POINTS_1; ?></div></div>','',430,190,'');break;
		case 'increasePoints':message('messages','<?=MNUUSER_INCREASEOINTS?>','<div class="font_size5"><div><?php echo MAINMENU_POINTS_2; ?></div></div>','',430,280,'');break;
		...
		case 'viewNoti':
			...
			break;
		case 'editProductTag':redir('update?tag='+opc[0]+'&product='+opc[1]);break;//actionsTags(10,opc[0]+'|0','controls/tags/actionsTags.controls.php','<?=$_SESSION['ws-tags']['ws-user']['code'].'|'.js_string(MNUTAG_ERRORNOISMYTAG).'|'.js_string(INDEX_ERRORHUBOHACK)?>');break;
		case 'tagsUser':
			if(opc[0]>0)
...
//			tagsUser(opc[0],'&current=personal&nolimit&tags_list&uid='+opc[1]+'');
		...
		break;
		case 'groupSuggest'	:suggestGroup('<?=GROUPS_SUGGESTGROUPTITLE?>',data);break;
...
function valida(form){//requerido=" label", opcional(tamanio="tamanio")
	...
	for (i=0;i<inputs.length;i++){
		msj='<b>'+inputs[i].getAttribute('requerido')+'</b>&nbsp;<?=REQUIREDPROFILE?>!\n';
...
function showTag(tag){
	...
	<?php
	if (PAYPAL_PAYMENTS) {
		echo "btnSponsor = (btn['sponsor']?
					'<li id=\"sponsors\" action=\"sponsor,'+tag['id']+'\" title=\"MNUTAG_TITLESPONSOR\"><span>MNUTAG_TITLESPONSOR</span></li>'
				:'')";
	}
	?>
	return(
	'<div tag="'+tag['id']+'" udate="'+tag['udate']+'">'+
		...
		'<div class="bg"></div>'+
		(tag['rid']?
			'<div class="redist"><span action="profile,'+tag['rid']+'" title="<?=js_string(USERPROFILE_PERSONALINFO)?>"><?=TIMELINE_REDISTRIBUTED?> '+tag['name_redist']+'</span></div>'
		:'')+
		'<div class="tagLink" action="comment,'+tag['id']+',<?=MNUTAG_TITLEMSGCOMMENTS?>"></div>'+
		'<div class="tag-icons">'+
		...
		'</div>'+
		'<div class="profilepic" action="profile,'+tag['uid']+'" title="<?=js_string(USERPROFILE_PERSONALINFO)?>"></div>'+
		'<div class="profile" action="profile,'+tag['uid']+'" title="<?=js_string(USERPROFILE_PERSONALINFO)?>"></div>'+
		((tag['product']||tag['typeVideo'])?
			'<div class="extras"><div>'+
				...
				:'')+
				(tag['typeVideo']?
					'<span class="'+tag['typeVideo']+'" action="video,'+tag['video']+'" title="<?=WATCH_VIDEO?>"></span>'
				:'')+
			'</div></div>'
		:'')+
		(isLogged()?
		'<menu>'+
			...
			'<ul>'+
				...
				:'')+(btn['redist']?
					'<li id="redistr" action="redist,'+tag['id']+'" title="<?=MNUTAG_TITLEREDISTRIBUTION?>"><span><?=MNUTAG_TITLEREDISTRIBUTION?></span></li>'
				:'')+(btn['share']?
					'<li id="share" action="share,'+tag['id']+'" title="<?=MNUTAG_TITLESHARE?>"><span><?=MNUTAG_TITLESHARE?></span></li>'
				:'')+btnSponsor+(btn['trash']?
					'<li id="trash" action="trash,'+tag['id']+'" title="<?=MNUTAG_TITLEREMOVE?>"><span><?=MNUTAG_TITLEREMOVE?></span></li>'
				:'')+((tag['product'])?(btn['edit']?
					'<li id="edit" action="editProductTag,'+tag['id']+','+tag['product']['id']+'" title="<?=MNUTAG_TITLEEDIT?>"><span><?=MNUTAG_TITLEEDIT?></span></li>'
				:''):(btn['edit']?
					'<li id="edit" action="editTag,'+tag['id']+'" title="<?=MNUTAG_TITLEEDIT?>"><span><?=MNUTAG_TITLEEDIT?></span></li>'
				:''))+(btn['report']?
					'<li id="report" action="report,'+tag['id']+'" title="<?=MNUTAG_TITLEABUSE?>"><span><?=MNUTAG_TITLEABUSE?></span></li>'
				:'')+
			'</ul>'+
		...
		$.ajax({
			...
			success:function(data){
				...
				if(data['tags']&&data['tags'].length>0){
					...
					act.more=false;
					if(current=='group'){
						$(layer).html('<div class="messageAdver"><?=GROUPS_MESSAGE_TAGS?>&nbsp;<?=GROUPS_MESSAGE_NEWTAG?><a href="'+BASEURL+'creation?group='+opc['grupo']+'"><?=GROUPS_MENUADDTAG?></a></div>');
					}else{
						if(idsearch=='1'){
							$(radiobtn).hide();
							$(layer).html('<div class="clearfix"></div><div class="messageNoResultSearch"><?=SEARCHALL_NORESULT?><span style="font-weight:bold"> '+search+'</span> <br><span style="font-size:12px"><?=SEARCHALL_NORESULT_COMPLE?></span></div>');
						}else{
							$(layer).html('<div class="clearfix"></div><div class="messageNoResultSearch more"><?=NORESULT_TIMELINE?></div>');
						}
					}
				}else if(action=='more'){
					$(layer).append('<div class="clearfix"></div><div class="messageNoResultSearch more"><?=NORESULT_MORETIMELINE?></div>');
				}
			},
			complete:function(){
				if (opc.pCont){
...
function trash(tag){
	...
	//actionsTags(6,opc+'|0',,'<?=js_string(MNUTAG_TITLEREMOVESSSS)?>|timeLine');
}
			...
			if(dato){
				out+='<div class="divYourFriendsSuggestPhoto">'+
					...
					'<div class="divYourFriendsSuggestInfo">'+
						...
						'</div>'+
						'<div class="right">'+
							'<input user="'+dato['id_friend']+'" class="ui-button ui-widget ui-state-default ui-corner-all" style="font-size: 9px; padding: 2px 5px; margin-left: 3px;" type="button" value="<?=USER_BTNLINK?>"  action="linkUser,'+id_capa+','+dato['id_friend']+',,.contentSuggestFriends">'+
					'</div>'+
				...
				if(data['cad']!=''){
					...
					$(id_capa).html('<div class="messageInviteSuggest"><?=INVITED_SUGGETSFRIENDS?></div>');
				}
			}
		}
	});
	...
	function showComment(comment){return(
		'<li comment="'+comment['id']+'">'
			...
			+'<div class="text">'
				...
				+'<span class="name" title="<?=COMMENTS_FLOATHELPVIEWPROFILEUSER?>" >'+comment['nameUser']+'</span>'
				+(!comment['subComment']?'<p>'+comment['comment']+'</p>':
					'<p>'+comment['subComment']+'</p>'
					+'<p style="display:none;">'+comment['comment']+'</p>'
					+'<span class="more"><?=USER_BTNSEEMORE?></span>'
				)
			+'</div>'
		+'</li>'
	);}
...
function actionGroup(id,action,get,obje){
		...
		success	:	function (data) {
			switch (action) {
				...
				case 4:
					if (data['leave']=='leave'){
						asigAdmin('<?=GROUPS_MENULEAVEGROUPLEGEND?>',id);
					...
					if (obje.btnJ){
						if(data['accept']=='true'){
							if (data['invite']=='no-invite'){
								$.dialog({
									title	: '<?=SIGNUP_CTRTITLEMSGNOEXITO?>',
									content	: '<?=GROUPS_ACEP_INVIT_NOT_INVIT?>',
									height	: 200,
									close	: function(){
										location.reload();
									}
...
function det_info(data){
    return ('<article class="info side-box imagenSug">'+
            '<header><span><?=SIGNUP_CTRTITLEALERT?></span></header>'+
            '<div>'+
                '<ul>'+
                    '<li><label><?=PRIVACY?>: </label>'+data['etiquetaPrivacidad']+'</li>'+
                    '<li><label><?=GROUPS_NEWORIENTED?>: </label>'+data['des_o']+'</li>'+
                    '<li><label><?=MAINMNU_CATEGORY?>: </label>'+data['cname']+'</li>'+
                    '<li><a><?=STORE_VIEWDETAILS?></a></li>'+
                '</ul>'+
            '</div>'+
        	'<strong></strong>'+
        	'<div class="clearfix"></div></article>'+
            '<article class="member side-box imagenSug">'+
            '<header><span><?=GROUPS_MEMBERSTITLE?></span></header>'+
            '<div>'+
                '<ul>'+
                    '<li><label><?=GROUPS_CREATOR?>: </label>'+data['name_create']+'</li>'+
                    '<li><label><?=GROUPS_NEWADMINISTRATOR?>: </label>'+data['num_admin']+'</li>'+
                    '<li><label><?=GROUPS_MEMBERSTITLE?>: </label>'+data['num_members']+'</li>'+
                    '<li><a><?=STORE_VIEWDETAILS?></a></li>'+
                '</ul>'+
            '</div>'+
        	'<strong></strong>'+
    ...
    var conten='<div id="dialog_detail_group">'+
            ...
            '<div class="side-box">'+
                '<ul>'+
                    ...
                    '<li><label><?=GROUPS_NEWORIENTED?>: </label>'+data['des_o']+'</li>'+
                    '<li><label><?=MAINMNU_CATEGORY?>: </label><img src="'+data['cphoto']+'" alt="Group Icons" title="'+data['ctitle']+'" width="30" height="30"/>'+data['cname']+'</li>'+
                    '<li><label><?=SIGNUP_LBLBUSINESSSINCE_FIELD?>: </label>'+data['date']+'</li>'+
                    '<li><label><?=GROUPS_DATE_JOIN?>: </label>'+data['date_join']+'</li>'+
                    '<li><label><?=GRP_DESCRIPTION?>: </label><p>'+data['des']+'</p></li>'+
                '</ul>'+
            '</div>'+
        	'<div class="clearfix"></div></div>';
    message('#default','<?=NOTIFICATIONS_GROUPSTAGMSJUSERLINK?>',conten,'',500,400);
}
				...
				success	:	function (data) {
					if (opc.actual==''){
						...
						if (data['body']){
                            ...
                            $('.ui-dialog-title').html('('+opc.total+') <?=GROUPS_MEMBERSTITLE?>')
						}else{
							$(opc.layer).html('<div class="messageAdver"><?=SEARCHALL_NORESULT?></div>');
                                               //+'<div><input name="btnCloseMembers" id="btnCloseMembers" type="button" value="<?=GROUPS_INVITEFRIENDSBTNBACK?>" style="float:right;margin: 1em;"></div></div>');
						}
...
function bodyListProd(prod,obje,i){
	...
	if (prod['type_user']=='1'){
		if (prod['id_user']=='427'){ footerDiv='<span class="footer color_orange"><?=STORE_TYPE_S?></span>'; }
		else {footerDiv='<span class="footer color_blue"><?=STORE_TYPE_1?></span>'; }
	}else if(prod['type_user']=='0'){ footerDiv='<span class="color_green footer"><?=STORE_TYPE_0?></span>'; }
	...
	if (prod['raffle'] && !obje.noBorde){
		...
		attrR='r="1"';
		var inputFooter='';
		if (prod['stock']=='0'){ inputFooter='<span class="nameSP" stock="0"><?= formatoCadena(STORE_STOCK)?>: 0</span>'; }
		else{
			if(obje.idUsr!=''){ inputFooter+='<span class="nameSP" action="newRaffle,'+prod['id']+'"><?=PRODUCTS_NEW_RAFFLE?></span>'; }
			inputFooter='<br><span class="nameSP" action="createTag,?product='+prod['id']+'"><?=MAINMNU_CREATETAG?></span>';
		}
		inputCreate='<div class="inputCreateRaffle">'
					...
					+'<div class="clearfix"></div>'
					+inputFooter
					+'<br><span class="nameSP" action="detailProd,'+prod['id']+'"><?=STORE_VIEWDETAILS?></span>'
				+'</div>'
	}else if (!obje.noBorde){
		attrR='r="2"';
		miniCar='<div class="miniCarStore">'
					+'<div title="<?=STORE_DETPRDDBTNADD?>" class="bg-add" h="'+prod['id']+'"></div>'
				+'</div>';
	}
	lst =	'<li p="1" '+actionLi+' class="'+clasesLi+'" '+attrR+'>'
				...
				+'<div '+actionDiv+' class="detail_store_product">'
					...
					+'<span class="priceSP"><?=PRODUCTS_PRICE?>:</span><span> '+(prod['pago']==1?' $'+cost:cost+' <?=STORE_TITLEPOINTS?>')+'</span><br>'
					+'<span class="footer"><?=STORE_CATEGORIES2?>: '+prod['category']+((prod['subCategory'])?'<br> <?=STORE_CATEGORIES3?>: '+prod['subCategory']+'</span>':'')+'<br>'
					+footerDiv
				+'</div>'
				+inputCreate+''+miniCar
			+'</li>';
...
function storeListProd(layer,get,search){
	...
	//$(layer).html('<span class="store-span-loader"><?=JS_LOADING.' '.PRODUCTS_LIST?></span>&nbsp;&nbsp;<img src="css/smt/loader.gif" width="25" height="25" />');
			...
			if(prod){
				...
				if(data['hash']){
					...
					out +='<article id="storeTagsRealted" class="side-box">'
							+'<header><span><?=STORE_RELATEDTAGS?></span></header>';
				...
				if(search!=''){
					...
					lst='<div class="messageNoResultSearch"><?=SEARCHALL_NORESULT?><span id="valSearch"> '+search+'</span> <br><span style="font-size:12px"><?=SEARCHALL_NORESULT_COMPLE?></span></div>'
					+'<div class="ui-single-box-title"><?=EDITFRIEND_VIEWTITLESUGGES?></div>';
					option = '<div class="messageNoResultSearch"><?=SEARCHALL_NORESULT?><span style="font-weight:bold"> '+search+'</span> <br><span style="font-size:12px"><?=SEARCHALL_NORESULT_COMPLE?></span></div>';
					storeListProd('.product-list.sugest','?rand=1&');
				}else{
					if (layer!='.product-list.sugest'){
						lst='<div class="noStoreProductsList messageAdver"><span><?=NOSTORE_MESSAGE?></span>'+(data['empre']?', <?=STORE_NOSTOREMESSAGE?>':'')+'</div>'
							+'<div class="ui-single-box-title"><?=EDITFRIEND_VIEWTITLESUGGES?></div>';
						storeListProd('.product-list.sugest','?rand=1&');
...
//			$('.button').button();
			if (data['asoWish'] && layer!='.product-list.sugest'){
				...
				lst += '</ul>';
				$('div.product-list.produc_sugg').html('<div class="ui-single-box-title" ><?=STORE_WISH_ASO?></div>'
														+'<div>'+lst+'</div>'
														+'<div class="clearfix"></div>');
			}
			if (data['asoSrh'] && layer!='.product-list.sugest'){
				...
				lst += '</ul>';
				$('div.product-list.produc_suggSrh').html('<div class="ui-single-box-title" ><?=STORE_SRH_ASO?></div>'
														+'<div>'+lst+'</div>'
														+'<div class="clearfix"></div>');
			}
		}
		...
		success: function(data){
			...
			if (prod){
				...
				for(var i=0;i<prod.length;i++){
					...
					lst +=	'<li p="1" '+((prod[i]['my'])?'action="newRaffle,'+prod[i]['id']+','+prod[i]['id_raffle']+'"':'action="detailProd,'+prod[i]['id']+',raffle"')+' class="'+((limt==1)?((i==(prod.length-1))?'border-botton-store':''):((limt==2)?(i>=(prod.length-2))?'border-botton-store':'':((limt==0)?(i>=(prod.length-3))?'border-botton-store':'':'')))+' '+(((i+1)%3==0)?'border-r-store':'')+'">'
								...
								+'<div class="detail_store_product">'
									...
									+'<span><?=PUBLICITY_TITLETABLE_COST?>: '+cost+' <?=STORE_TITLEPOINTS?></span><br>'
									+'<span class="footer"><?=PRODUCTS_FECHA_INI?>: '+prod[i]['start_date']+'</span><br>'
									+((prod[i]['end_date'])?'<span class="footer"><?=PRODUCTS_FECHA_END?>: '+prod[i]['end_date']+'</span>':'<span class="footer"><?=PRODUCTS_USER_CANT?>: '+prod[i]['cant_users']+'</span>')
								+'</div>'
							+'</li>';
				}
				lst += '</ul>';
				...
				if (layer!='.product-list .product-list'){
					lst='<div class="noStoreProductsList messageAdver"><span><?=NOSTORERAFFLE_MESSAGE?></span> <?=($_SESSION['ws-tags']['ws-user']['type']==1?', '.STORE_NOSTOREMESSAGERAFFLE:'')?></div>'
				...
				if(module=='store'){
					...
					for(var i=0;i<prod.length;i++){
						lst +=	'<li action="detailProdRaffle,'+prod[i]['id']+'" class="'+((limt==1)?((i==(prod.length-1))?'border-botton-store':''):((limt==2)?(i>=(prod.length-2))?'border-botton-store':'':((limt==0)?(i>=(prod.length-3))?'border-botton-store':'':'')))+' '+(((i+1)%3==0)?'border-r-store':'')+'">'
									...
									+'<div>'
										+'<span class="nameSP">'+prod[i]['name']+'</span><br>'
										+'<span><?=PUBLICITY_TITLETABLE_COST?>: '+prod[i]['points']+' <?=STORE_TITLEPOINTS?></span><br>'
										+'<span class="footer"><?=PRODUCTS_FECHA_INI?>: '+prod[i]['start_date']+'</span><br>'
										+'<span class="footer"><?=PRODUCTS_USER_CANT?>: '+prod[i]['cant_users']+'</span>'
									+'</div>'
								+'</li>';
					}
				}
...
//								$('.button').button();
							...
							$('#list_orderProduct_wish').html('<div class="messageAdver"><?=STORE_NO_WL?></div>');
							$('div.ui-single-box').removeAttr('style');
						}
					}
...
function deleteOrderC(get,obj){
	$.dialog({
		title:'<?=STORE_DELETESHOPPINGTITLE?>',
		width:270,
		height:200,
		resizable:false,
		open:function(){
			$(this).html('<?=js_string(STORE_DELETESHOPPING)?>');
		},
		buttons:[{
			text:'<?=JS_NO?>',
			click:function(){$(this).dialog('close');}
...
function bodyShopingCar(data,i){
	...
	var lst='<li class="carStore liVoid'+i+' '+((data['stock']==0)?'noST':'')+'">'
			...
			+'			<h5>'+data['nameUser']+'</h5>'
			+'			<div class="anytext"><?=USER_LBLFOLLOWERS?> ('+data['admirers']+')</div>'
			+'			<div class="anytext"><?=USER_LBLFRIENDS?> ('+data['admired']+')</div>'
			+'		</div>'
			+'		<div class="clearfix"></div>'
			+'		<span class="footer">'+data['nameC']+((data['nameSC'])?((data['place'])?' | '+data['nameSC']+'</span>':'</span>'):'</span>')
			+'		<br><span class="deleteItemCar" action="deleteItemCar,'+data['mId']+',car,'+(data['sale_points']*data['cant'])+'"><?=NEWTAG_HELPDELETEBACKGROUNDTEMPLATE?></span>'
			+'		<span class="addToWish" h="'+data['mId']+'"><?=utf8_encode(STORE_WISH_LIST_MOVE)?></span>'
			+'	</div>'
			+'</li>'
			+'<li class="carStorePrice liVoid'+i+' '+((data['stock']==0)?'noST':'')+'">'
			+'	<div class="info-top-p">'+((data['formPayment']=='1')?'$ '+cost:cost+' <?=STORE_TITLEPOINTS?>')+'</div>'
			+'</li>'
			...
			+'	'+((data['place']=='1')?
					((data['stock']>0)?
						'<div class="info-top-p">'
							...
							+'</select>'
						+'</div>'
						:'<em class="info-top-p"><?=TAGS_WHENTAGNOEXIST?></em><input type="hidden" class="cant-product" linia="'+data['mId']+'" value="'+data['cant']+'">')
					:'')
...
function sumaryShopingCar(data){
	if (data['totalpoints']!=0 || data['totalmoney']!=0){
		...
		$('#headerStoreCar .totalPointsSC #moveTotal').val(0);
		$('#headerStoreCar .totalPointsSC #spanTotalPrice').html('');
		$('#headerStoreCar .totalPointsSC #totalPrice').html('<?=NOSTORE_MESSAGE?>');
//		$('#buyOrder').button('disable');
	}
	$('#headerStoreCar').css('display','block');
}
function makeShopingCar(){
	$('#list_orderProduct').html('<span class="store-span-loader"><?=JS_LOADING.' '.PRODUCTS_LIST?></span>&nbsp;&nbsp;<img src="css/smt/loader.gif" width="25" height="25" />');
	$$.ajax({
		type: 'GET',
...
//										$('.button').button();
				...
				if(data['wish']['body']){
					...
					$('#list_orderProduct_wish').html('<div class="ui-single-box-title"><?=STORE_WISH_LIST?></div>'
...
function makeListWish(){
	$('#list_orderProduct_wish').html('<span class="store-span-loader"><?=JS_LOADING.' '.PRODUCTS_LIST?></span>&nbsp;&nbsp;<img src="css/smt/loader.gif" width="25" height="25" />');
	$$.ajax({
		type: 'GET',
		...
		$.ajax({
			...
			success:function(data){
					if (data['datosCar']=='checked'){
						$.dialog({
							title:'<?=STORE_ORDERSENT?>',
							resizable:false,
							width:270,
							height:190,
							open:function(){
								$(this).html('<?=js_string(STORE_MSG_CHECKOUT)?>');
							},
							close:function(){
						...
						message('<?=BUY?>','<?=RESET_TITLEALERTEMAILPASSWORD?>','<?=STOREMESSAGENOPOINTSFORBUY?>','',300,200);
				...
				for(var i=0;i<prod.length;i++){
					...
					lst +=	'<li action="detailProd,'+prod[i]['id']+'" '+((i<=2)?'class="border-top-store"':'')+'>'
								...
								+'<div>'
									...
									+'<span><?=PUBLICITY_TITLETABLE_COST?>: '+(prod[i]['pago']==1?' $'+cost:cost+' <?=STORE_TITLEPOINTS?>')+' <?=STORE_TITLEPOINTS?></span><br>'
									+'<span class="colorSpanStore"><?=STORE_VIEWDETAILS?></span><br><br>'
								+'</div>'
								+'<div class="clearfix"></div>'
							+'</li>';
				}
...
function addNewRaffleStore(getidProduct,getidRaffle) {
	...
	if(getidRaffle==''){
		buttonss['<?=JS_CONTINUE?>']=function(){
				if (valida('frmProducts')) $('#frmProducts').submit();
			};
	}
	$.dialog({
		title:'<?=PRODUCTS_NEW_RAFFLE?>',
		resizable:false,
		width:500,
		height:420,
		modal:true,
					...
					for(var i=0;i<data['datosCar'].length;i++){
						...
						if (idOrder!=data['datosCar'][i]['idOrder']){
							lst+='<li class="purchaseorder border-botton-store '+(i!=0?'border-t-store':'')+'">'
								+'	<div>'
								+'		<strong><?=STORE_PURCHASETITLE?></strong>'
								+		((data['datosCar'][i]['pago']=='1')?'&nbsp;&nbsp;<span class="nameSP"><a href="'+BASEURL+'shoppingcart"><?=STORE_PROCEED?></a></span>'
																			+'&nbsp;&nbsp;<span class="nameSP"><a class="delete" h="1" href="javascript:void(0);"><?=PRODUCTS_DELETE?></a></span>':'')
								+		((data['datosCar'][i]['pago']=='11')?'&nbsp;&nbsp;<span class="nameSP"><a class="pay" h="'+data['datosCar'][i]['idOrderM']+'" href="javascript:void(0);"><?=JS_PAY?></a></span>'
																			+'&nbsp;&nbsp;<span class="nameSP"><a class="delete" h="'+data['datosCar'][i]['idOrderM']+'" href="javascript:void(0);"><?=PRODUCTS_DELETE?></a></span>':'')
						...
						lst +=	'<li class="carStore border-botton-store">'
								...
								+'			<span class="footer"><span class="nameSP"><?=formatoCadena(STORE_FROM)?>: </span>'+data['datosCar'][i]['inicio']+'</span><br>'
								+'			<span class="footer"><span class="nameSP"><?=formatoCadena(STORE_TO)?>: </span>'+data['datosCar'][i]['fin']+'</span>'
								+'	</div>'
								+'</li>'
								+'<li class="carStorePrice border-botton-store">'
								+'	<div class="info-top-p">'+((data['datosCar'][i]['product_formPayment']=='1')?'$ '+cost:cost+' <?=STORE_TITLEPOINTS?>')+'</div>'
								+'</li>'
								+'<li class="carStoreQuantity border-botton-store">'
								+'	<div class="info-top-p"><?=QUANTITYSTORE?>:<br/>'+data['datosCar'][i]['product_cant']+'</div>'
								+'</li>';
...
//				$('.button').button();
				...
				lst='<div class="product-list"><div class="noStoreProductsList messageAdver"><span><?=NOORDERS_MESSAGE?></span></div></div>';
			}
			$('#list_orderProduct').html(lst);
		}
	});
			...
			if(data['datosCar'].length>0){
				...
				for(var i=0;i<data['datosCar'].length;i++){
					...
					lst +=	'<li class="carStoreDetails '+(i==(data['datosCar'].length-1)?'border-botton-store':'')+'">'
							+'	<div class="lis_product_store_details">'
							+'		<span class="nameSP" ><?=STORE_BUYER.': '?></span><br>'
							+'		<div title="<?=STORE_BUYER?>" action="profile,'+data['datosCar'][i]['buyer']+','+data['datosCar'][i]['name_user']+'">'
					...
					lst+=	'	</div>'
							...
							+'	<div class="info-top-p"><span class="colorSpanStore" action="viewDetailsMySales,'+data['datosCar'][i]['idOrderM']+'"><?=STORE_VIEWDETAILS?></span></div>'
							+'</li>';
...
function detailsSalesProcessed(get){
	...
	$.ajax({
		...
		success: function(data){
			if (data['datosCar'].length>0){
				...
				for (i=0;i<data['datosCar'].length;i++){
					...
					lst +=	'<li class="carStore border-botton-store '+(i==0?'':'border-t-store')+'">'
							...
							+'	<div class="lis_product_store_details">'
							+'		<div class="clearfix"></div>'
							+'			<strong><?=STORE_SALES_DETAILS_PRODUCTS?></strong><br/>'
							+'			<span class="footer"><span  class="nameSP"><?=STORE_TITLENAME?>: </span><strong><span action="detailProd,'+data['datosCar'][i]['product_id']+',dialog-order">'+data['datosCar'][i]['product_name']+'</span></strong><br>'
							+'			<span class="footer"><span class="nameSP"><?=STORE_CATEGORIES2?></span>: '+data['datosCar'][i]['product_category']+((data['datosCar'][i]['product_subCategory'])?((data['datosCar'][i]['product_place'])?'<br><span class="nameSP"><?=STORE_CATEGORIES3?></span>: '+data['datosCar'][i]['product_subCategory']+'</span>':'</span>'):'</span>')+'<br>'
							+'			<strong><?=STORE_SALES_DETAILS_PUBLICATIONS?></strong><br/>'
							+'			<span class="footer"><span class="nameSP"><?=formatoCadena(STORE_FROM)?>: </span>'+data['datosCar'][i]['inicio']+'</span><br>'
							+'			<span class="footer"><span class="nameSP"><?=formatoCadena(STORE_TO)?>: </span>'+data['datosCar'][i]['fin']+'</span>'
							+'	</div>'
							+'</li>'
							+'<li class="carStorePrice border-botton-store '+(i==0?'':'border-t-store')+'">'
							+'	<div class="info-top-p"><strong><?=PRODUCTS_PRICE?></strong>:<br>'
							+	((data['datosCar'][i]['product_formPayment']=='1')?'$ '+cost:cost+' <?=STORE_TITLEPOINTS?>')+'</div>'
							+'	'+((data['datosCar'][i]['pago']=='12')?'<br><br><input type="button" action="revendProduct,'+data['datosCar'][i]['product_id']+'" value="<?=formatoCadena(STORE_RESELL)?>">':'')
							+'</li>'
							+'<li class="carStoreQuantity border-botton-store '+(i==0?'':'border-t-store')+'">'
							+'	<div class="info-top-p"><strong><?=QUANTITYSTORE?>:</strong></br>'+data['datosCar'][i]['product_cant']+'</div>'
							+'</li>';
				}
				lst+='</ul></div>';
			}else{
			...
			$.dialog({
				title:'<?=STORE_SALES_DETAILS_SALES?>',
				resizable:false,
				width:700,
				height:520,
				modal:true,
		...
		success: function(data){
			if (data['datosCar'].length>0){
				var i,titleFreP='<div style="display:inline-block;">'
								+'<select class="chzn-b">'
									+'<option value="all"><?=GROUPSALLSELECT?></option>'
									for(var i=0;i<data['datosCar'].length;i++){
										switch(data['datosCar'][i]['tipo']){
											case '11': titleFreP+='<option value="pend"	'+(radio=='pend'?'selected':'')+'><?=STORE_ORDERS_PENDING?></option>'; break;
											case '12': titleFreP+='<option value="fins"	'+(radio=='fins'?'selected':'')+'><?=STORE_ORDERS_FINALIZED?></option>'; break;
										}
									}
				titleFreP+='</select>'
						+'<div>'
...
function chooseProducts(tag){
	...
	$.dialog({
		title: '<?=formatoCadena(STORE_SELECT_A_PRODUCT)?>',
		resizable: false,
		width: 700,
		height: 520,
		modal: true,
		open:function(){
			if(tag){
				$(this).html('<div id="selectProductsTags">'
								+'<div class="product-list"></div>'
								+'<div id="loaderStoreDialog" style="display:none;width: 555px;float: left;"><span class="store-span-loader"><?=JS_LOADING.' '.PRODUCTS_LIST?></span>&nbsp;&nbsp;<img src="css/smt/loader.gif" width="25" height="25" /></div>'
							+'</div>');
...
function bodyfriends(friends,Link,unLink){
	...
	if(friends['username']){
		username='<span class="titleField"><?=USERS_BROWSERFRIENDSLABELEXTERNALPROFILE?>:</span>&nbsp;<a style="color:#ccc; font-size:12px;" href="'+BASEURL+friends['name_user']+'" onFocus="this.blur();" target="_blank">"'+BASEURL+friends['name_user']+'"</a>'
				+ '<div class="clearfix"></div>';
	}
	if(friends['country']!=''){
		country='<span class="titleField"><?=USERS_BROWSERFRIENDSLABELCOUNTRY?>:&nbsp;</span>'+friends['country']
				+ '<div class="clearfix"></div>';
	}
	...
	return('<div id="div_'+md5(friends['id_friend'])+'" class="divYourFriends">'
		...
		+			'</a><br>'
		+			username
		+			'<span class="titleField"><?=SIGNUP_LBLEMAIL?>: </span>'+friends['email']
		+			'<div class="clearfix"></div>'
		+			'<span class="titleField"><?=USER_LBLFOLLOWERS?>('+friends['followers_count']+')</span>'
		+			'<span class="titleField"><?=USER_LBLFRIENDS?>('+friends['friends_count']+')</span><div class="clearfix"></div>'
		+			country
		+		'</div>'
		+		'<div style="height:70px; width:0px; float: right; text-align: right;">'
		+			'<input style="margin-top: 20px;font-size:10px;'+Link+'" name="btn_link_'+md5(friends['id_friend'])+'" id="btn_link_'+md5(friends['id_friend'])+'" type="button" value="<?=USER_BTNLINK?>" action="linkUser,,'+md5(friends['id_friend'])+'" />'
		+			'<input style="margin-top: 20px;font-size:10px;'+unLink+'" name="btn_unlink_'+md5(friends['id_friend'])+'" id="btn_unlink_'+md5(friends['id_friend'])+'" type="button" value="<?=USER_BTNUNLINK?>" action="linkUser,,'+md5(friends['id_friend'])+',animate" />'
		+		'</div>'
		+	'</div>'
		+	'<div class="clearfix"></div>'
		+ '</div>');
...
function bodygroups(groups){
	...
	if(groups['privacy']==3){
		...
		btn=	'<div>'
				+		'<input action="groupSuggest,'+groups['id']+'" type="button" value="<?=GROUPS_SUGGESTGROUP?>" name="suggestGroup'+groups['id']+'" id="suggestGroup'+groups['id']+'">'
				+		'<a href="'+BASEURL+'groupsDetails?grp='+groups['id']+'" class="viewGroup button"><?=GROUPS_VIEWTHEGROUP?></a>'
				+	'</div>'
				+ '<div class="clearfix"></div>';
		...
		if(groups['userInGroup']==0){
			if(groups['buttonGroup']==0){
				btn   = '<input type="button" value="<?=GROUPS_JOINTOTHEGROUP?>" name="joinGroup'+groups['id']+'" id="joinGroup'+groups['id']+'" action="groupsAction,'+groups['id']+'">'
				+	'<div id="autoriGr'+groups['id']+'" class="messageSuccessGroupo" style="display: none"><?=JS_GROUPS_WAITAPPROBATION?></div>'
				atc	=	'action="groupsAction,'+groups['id']+'"';
			}else if(groups['buttonGroup']==1){
				btn   =		'<div id="btnJoinViewroup'+groups['id']+'">'
					+		'<input action="groupSuggest,'+groups['id']+'" type="button" class="viewGroup suggestGroup'+groups['id']+'" value="<?=GROUPS_SUGGESTGROUP?>">'
					+		'<input action="groupsDetails,'+groups['id']+'" type="button" class="viewGroup" value="<?=GROUPS_VIEWTHEGROUP?>"></div>';
				atc	='action="groupsDetails,'+groups['id']+'"';
				...
				btn   =		'<div id="btnJoinViewroup'+groups['id']+'">'
					...
					+	'<span class="msg" id="msg'+groups['id']+'"><?=INVITE_GROUP_TRUE?></span>'
					+	'<input type="button" value="<?=GROUPS_JOINTOTHEGROUP?>" name="joinGroup'+groups['id']+'" id="joinGroup'+groups['id']+'" action="groupsAction,'+groups['id']+'" style="display: none">'
					+	'<div id="autoriGr'+groups['id']+'" class="messageSuccessGroupo" style="display: none"><?=JS_GROUPS_WAITAPPROBATION?></div>'
					+	'</div>';
			...
			btn = '<div class="messageSuccessGroupo"><?=JS_GROUPS_WAITAPPROBATION?></div>'
			+ '<div class="clearfix"></div>';
		}
	}
