	...
	'<div tag="'+tag['id']+'" udate="'+tag['udate']+'">'+
		...
		'<menu>'+
			...
			'<ul>'+
				...
				:''))+(btn['report']?
					'<li id="report" action="report,'+tag['source']+'" title="Hide or report a tag"><span>Hide or report a tag</span></li>'
				:'')+
			'</ul>'+
			...
			success	:	function (data) {
				if (opc.actual==''){
					...
					if (data['html']){
						...
						if (opc.actual<opc.total) $(opc.layer).append('<a class="plus" id="seemore">See more</a><div class="clearfix"></div>');
					}else{
		...
		success: function(data){
			...
			if (prod){
				...
				if (layer!='.product-list .product-list'){
					lst='<div class="noStoreProductsList messageAdver"><span>There are no free products available</span>'+(data['adtb']?', If you want to add <span id=\"clickNewRaffle\">click here</span>':'')+'<div class="product-list"></div>';
				}
			}
			$(layer).html(lst);
		}
