function pageAction(action,data){
	...
	switch(action){
		...
		case 'increasePoints':message('messages','Â¿CÃ³mo aumentar mis puntos?','<div class="font_size5"><div><?$lang["MAINMENU_POINTS_2"]?></div></div>','',430,280,'');break;
...
function showTag(tag){
	...
	'<div tag="'+tag['id']+'" udate="'+tag['udate']+'">'+
		...
		:'')+
		'<div class="tagLink" action="comment,'+tag['id']+',Comparte tu opiniÃ³n sobre esta tag."></div>'+
		'<div class="tag-icons">'+
		...
		'<menu>'+
			...
			'<ul>'+
				...
				:'')+(btn['redist']?
					'<li id="redistr" action="redist,'+tag['id']+'" title="Re DistribuciÃ³n"><span>Re DistribuciÃ³n</span></li>'
				:'')+(btn['share']?
		...
		$.ajax({
			...
			success:function(data){
				...
				if(data['tags']&&data['tags'].length>0){
					...
					act.more=false;
					if(current=='group'){
						$(layer).html('<div class="messageAdver">No hay tags creadas.&nbsp;Para crear una tag, haz clic aqui<a href="'+BASEURL+'creation?group='+opc['grupo']+'">AÃ±adir Tag</a></div>');
					}else{
						if(idsearch=='1'){
							$(radiobtn).hide();
...
function confirJOINGroups(opc) {
	$.dialog({
		title	: 'InformaciÃ³n',
		content	: 'Â¿Deseas unirte a este grupo?',
		height	: 200,
		close	: function(){
			$(this).dialog('close');
		},
	...
	$.dialog({
		...
		open: function() {
			$(this).html('<div id="bordeasigAdmin">'
						...
						+'					</div>'
						+'					<div class="text">'
						+'						Usted es un administrador de este grupo. Si el grupo queda sin administrador, serÃ¡ eliminado. Â¿EstÃ¡ seguro que desea abandonar el grupo?'
						+'					</div>'
						+'				</div>'
						+'					<div class="clearfix"></div>'
						+'			</div>'
...
function det_info(data){
    return ('<article class="info side-box imagenSug">'+
            '<header><span>InformaciÃ³n</span></header>'+
            '<div>'+
                '<ul>'+
                    '<li><label>Privacidad: </label>'+data['etiquetaPrivacidad']+'</li>'+
                    '<li><label>OrientaciÃ³n: </label>'+data['des_o']+'</li>'+
                    '<li><label>CategorÃ­a: </label>'+data['cname']+'</li>'+
    ...
    var conten='<div id="dialog_detail_group">'+
            ...
            '<div class="side-box">'+
                '<ul>'+
                    ...
                    '<li><label>OrientaciÃ³n: </label>'+data['des_o']+'</li>'+
                    '<li><label>CategorÃ­a: </label><img src="'+data['cphoto']+'" alt="Group Icons" title="'+data['ctitle']+'" width="30" height="30"/>'+data['cname']+'</li>'+
                    '<li><label>La fecha de su fundaciÃ³n: </label>'+data['date']+'</li>'+
                    '<li><label>Se unio el : </label>'+data['date_join']+'</li>'+
                    '<li><label>DescripciÃ³n: </label><p>'+data['des']+'</p></li>'+
                '</ul>'+
            '</div>'+
					...
					if (opc.actual==''){
						...
						if (data['body']){
							...
							$(opc.layer).html('<div class="messageAdver">Disculpe, no hay resultados para</div>');
                                               //+'<div><input name="btnCloseMembers" id="btnCloseMembers" type="button" value="Volver al Grupo" style="float:right;margin: 1em;"></div></div>');
						}
...
function bodyListProd(prod,obje,i){
	...
	if (prod['type_user']=='1'){
		if (prod['id_user']=='427'){ footerDiv='<span class="footer color_orange">Producto de Tagbum</span>'; }
		...
		success:function(data){
			...
			if(prod){
				...
				if(search!=''){
					...
					lst='<div class="messageNoResultSearch">Disculpe, no hay resultados para<span id="valSearch"> '+search+'</span> <br><span style="font-size:12px">Compruebe su termino de busqueda e intentelo de nuevo.</span></div>'
					+'<div class="ui-single-box-title">Sugerencias</div>';
					option = '<div class="messageNoResultSearch">Disculpe, no hay resultados para<span style="font-weight:bold"> '+search+'</span> <br><span style="font-size:12px">Compruebe su termino de busqueda e intentelo de nuevo.</span></div>';
	...
	$$.ajax({
		...
		console.log(data);
			if (data['del']!='1'){
				if (data['del']=='all'){
					if (obj.mod!='wish'){
						$('#list_orderProduct').html('<div class="messageAdver">Disculpe, no hay artÃ­culos disponibles en su carrito de compras.</div>');
...
function deleteOrderC(get,obj){
	$.dialog({
		...
		resizable:false,
		open:function(){
			$(this).html('Â¿Est&aacute;s seguro de eliminar esta orden?');
		},
		buttons:[{
			text:'No',
			click:function(){$(this).dialog('close');}
...
function makeShopingCar(){
	$('#list_orderProduct').html('<span class="store-span-loader">Cargando Lista de productos</span>&nbsp;&nbsp;<img src="css/smt/loader.gif" width="25" height="25" />');
	$$.ajax({
		type: 'GET',
...
//											$('.button').button();
											if (data['wish']['disable']){
												if ($('#list_orderProduct_wish .messageAdver').length>0){
													...
													$('#list_orderProduct_wish').prepend('<div class="ui-single-box-title">Lista de Deseos</div>'+
...
//										$('.button').button();
				...
				if(data['wish']['body']){
					...
					$('#list_orderProduct_wish').html('<div class="ui-single-box-title">Lista de Deseos</div>'
...
function makeListWish(){
	$('#list_orderProduct_wish').html('<span class="store-span-loader">Cargando Lista de productos</span>&nbsp;&nbsp;<img src="css/smt/loader.gif" width="25" height="25" />');
	$$.ajax({
		type: 'GET',
			...
			$.ajax({
				...
				success:function(data){
					if(data['datosCar']=='update'){
						...
						$.dialog({
							title	: 'InformaciÃ³n',
							content	: 'Algunos productos ya no estÃ¡n disponibles, su pedido fue modificado.',
			...
			success:function(data){
					if (data['datosCar']=='checked'){
						$.dialog({
							...
							height:190,
							open:function(){
								$(this).html('Verifique su bandeja de entrada para obtener mas informaciÃ³n al respecto.');
							},
							close:function(){
				...
				for(var i=0;i<prod.length;i++){
					...
					lst +=	'<li action="detailProd,'+prod[i]['id']+'" '+((i<=2)?'class="border-top-store"':'')+'>'
								...
								+'<div>'
									...
									+'<span>Costo: '+(prod[i]['pago']==1?' $'+cost:cost+' Puntos')+' Puntos</span><br>'
									+'<span class="colorSpanStore">Ver Detalles &raquo;</span><br><br>'
								+'</div>'
								+'<div class="clearfix"></div>'
							+'</li>';
				}
					...
					for(var i=0;i<data['datosCar'].length;i++){
						...
						if (idOrder!=data['datosCar'][i]['idOrder']){
							lst+='<li class="purchaseorder border-botton-store '+(i!=0?'border-t-store':'')+'">'
								...
								+'		<span class="right">Fecha: '+data['datosCar'][i]['dateOrder']+'</span>'
								+'	</div>'
								+'</li>';
						...
						lst +=	'<li class="carStore border-botton-store">'
								...
								+'		<div title="Vendedor" action="profile,'+data['datosCar'][i]['product_seller']+','+data['datosCar'][i]['product_name_user']+'">'
								...
								+'	<div class="info-top-p">'+((data['datosCar'][i]['product_formPayment']=='1')?'$ '+cost:cost+' Puntos')+'</div>'
								+'</li>'
								+'<li class="carStoreQuantity border-botton-store">'
								+'	<div class="info-top-p">Cantidad:<br/>'+data['datosCar'][i]['product_cant']+'</div>'
								+'</li>';
			...
			if(data['datosCar'].length>0){
				...
				for(var i=0;i<data['datosCar'].length;i++){
					...
					lst +=	'<li class="carStoreDetails '+(i==(data['datosCar'].length-1)?'border-botton-store':'')+'">'
							+'	<div class="lis_product_store_details">'
							+'		<span class="nameSP" >Comprador: </span><br>'
							+'		<div title="Comprador" action="profile,'+data['datosCar'][i]['buyer']+','+data['datosCar'][i]['name_user']+'">'
					...
					lst+=	'	</div>'
							...
							+'	<div class="info-top-p"><span class="colorSpanStore" action="viewDetailsMySales,'+data['datosCar'][i]['idOrderM']+'">Ver Detalles &raquo;</span></div>'
							+'</li>';
								...
								+'<select class="chzn-b">'
									...
									for(var i=0;i<data['datosCar'].length;i++){
										switch(data['datosCar'][i]['tipo']){
											case '11': titleFreP+='<option value="pend"	'+(radio=='pend'?'selected':'')+'>Pendientes</option>'; break;
											case '12': titleFreP+='<option value="fins"	'+(radio=='fins'?'selected':'')+'>Finalizadas</option>'; break;
										}
									}
				titleFreP+='</select>'
						+'<div>'
...
function bodyfriends(friends,Link,unLink){
	...
	if(friends['country']!=''){
		country='<span class="titleField">PaÃ­s:&nbsp;</span>'+friends['country']
				+ '<div class="clearfix"></div>';
	}
	...
	if(groups['privacy']==3){
		...
		if(groups['userInGroup']==0){
			if(groups['buttonGroup']==0){
				...
				+	'<div id="autoriGr'+groups['id']+'" class="messageSuccessGroupo" style="display: none">Esperar aprobaciÃ³n</div>'
			...
			btn = '<div class="messageSuccessGroupo">Esperar aprobaciÃ³n</div>'
			+ '<div class="clearfix"></div>';
		}
	}
