...
class receiveMail
{
	...
	function receiveMail($username,$password,$EmailAddress,$mailserver='localhost',$servertype='pop',$port='110',$ssl = false) //Constructure
	{
		...
		$this->email			=	str_replace('.','_',$EmailAddress);
		
		if($servertype=='imap')
		{
			if($port=='') $port='143'; 
			$strConnect='{'.$mailserver.':'.$port. '}INBOX.$this->email'; ///notls 
			
		}
		else
		{
			$strConnect='{'.$mailserver.':'.$port. '/pop3'.($ssl ? "/ssl" : "").'}INBOX.$this->email';//; 
		}
	...
	function connect() //Connect To the Mail Box
	{
		...
		if(!$this->marubox)
		{
			echo "Error: Connecting to mail server:".imap_last_error() ;
			exit;
		}
	}
...
//					if(!in_array($extension,$imagesAllowed)){// this is for the error when the file name has no extension
					...
					//}	
						...
						fwrite($fp,$message);
						fclose($fp);
						redimensionar($path.$name, $path.$name, 650);
						uploadFTP($name,"templates", '../');
