...
if(isset($_GET['payAcc'])){
	switch ($_GET['payAcc']) {
		case 'store':
			...
			if (!$noProduct){
				...
				with_session(function(&$sesion){ unset($sesion['havePaypalPayment'],$sesion['car']); });
	...
	if (!isset($custom) || $custom == '' && isset( $_GET['uid'] ) ) {
		...
		if( mysql_num_rows($planPurchase) <= 0){
			$incriptionCost = CON::getVal('SELECT cost_account_company FROM config_system WHERE id = 1');;
		}
		...
		$plan=CON::getRow('SELECT price,name FROM subscription_plans WHERE id = ?',array($_GET['plan']));
