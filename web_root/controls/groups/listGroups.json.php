...
if(isset($_GET['infoG'])){
	...
	$orienteds = CON::query("
		SELECT o.description AS des,o.id AS id
	...
	while($row=CON::fetchAssoc($category)){
		$row['name']=lan($row['name']);
		$info[]=$row;
	}
	$res['category']=$info;
	//tipo de privacidad que manejara el grupo
	$privacys = CON::query("
		SELECT p.id,p.name,p.description AS des
...
if($where!=''){
	...
	while($row=CON::fetchAssoc($results)){
		...
		//$cate_name = (empty($row['cname']) || strpos($row['cname'], 'GROUPS_CATEGORY_')>= 0 )?'General':$row['cname'];
		...
		if(isset($row['des_o'])){ $row['des_o']=formatoCadena(lan($row['des_o'])); }
		if(isset($_GET['profile'])){$row['name_create']=formatoCadena($row['name_create']); }
		$row['ctitle']=$row['cname'].': '.lan($row['csummary']);
