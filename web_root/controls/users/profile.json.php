...
with_session(function(&$sesion){
	$myId=$sesion['ws-tags']['ws-user']['id'];
	$code=$sesion['ws-tags']['ws-user']['code'];
	...
	if (isset($_GET['skipProgress'])){
		...
		return;
	}else unset($sesion['ws-tags']['ws-user']['progress']);
	...
	//		unlink($data['img']['tmp_name']);
		...
		$photo="img/users/$code/".$sesion['ws-tags']['ws-user']['photo'];
	...
	if($data['action']=='save'){
		...
		if ($sesion['ws-tags']['ws-user']['type']!='1'){
			if($data['firstname']==''){
		...
		if( $sesion['ws-tags']['ws-user']['full_name']!=$temporal ) {
			$name_change=true;
		...
		if( $sesion['ws-tags']['ws-user']['language']!=$data['lang'] ) {
			$updateLanguage=true;
		...
		if(count($n_bc)>0){
			$bc='
				UPDATE business_card SET
					home_phone	="'.$sesion['ws-tags']['ws-user']['home_phone'].'",
					work_phone	="'.$sesion['ws-tags']['ws-user']['work_phone'].'",
					mobile_phone="'.$sesion['ws-tags']['ws-user']['mobile_phone'].'"
				WHERE id="'.$n_bc['id'].'"
			';
		...
		#cambio de zip code
		if($data['zipcode']!=$sesion['ws-tags']['ws-user']['zip_code']){
	...
	if ($data['action']=='fileCover'){
		if( $data['cover']['error']==0 ) {
			...
			if( in_array($ext, $imagesAllowed) ) {
				$path  = "$config->relpath/img/users_cover/".$sesion['ws-tags']['ws-user']['code'].'/';//ruta para crear dir
				$photo = $sesion['ws-tags']['ws-user']['code'].'/'.md5(str_replace(' ', '', $data['cover']['name'])).'.jpg';
	...
	//retornando los cambios del profile
		if($name_change){
			...
			if(!fileExists(($config->local?RELPATH:$config->imgserver)."img/users/$code/".$sesion['ws-tags']['ws-user']['photo'])){
				$sesion['ws-tags']['ws-user']['updatePicture']=0;
	...
	$sesion['ws-tags']['ws-user']['pic']="img/users/$code/".$sesion['ws-tags']['ws-user']['photo'];
	$sesion['ws-tags']['ws-user']['paypal']=$data['paypal'];
