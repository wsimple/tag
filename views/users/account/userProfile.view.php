	...
	<div data-role="content" id="userProfile" class="userProfileStyle">
		...
		<ul class="list_inline">
			...
			<li style="margin-bottom:10px;">
				...
				<input type="button" id="followers" onclick="<?php if ($friend['followers_count']>0){ ?>friendsUser('<?=$friend[screen_name].' : '.USER_LBLFOLLOWERS?>','?uid=<?=$friend[id]?>&follower=1&fid=<?=$idUserprodile?>'); <?php } ?>"
					value="<?=mskPoints($friend['followers_count'])?>"/>

				<strong><?=USER_LBLFRIENDS?>: </strong>
				<input type="button" id="friends" onclick="<?php if ($friend['following_count']>0){ ?>friendsUser('<?=$friend[screen_name].' : '.USER_LBLFRIENDS?>','?uid=<?=$friend[id]?>&fid=<?=$idUserprodile?>'); <?php } ?>"
